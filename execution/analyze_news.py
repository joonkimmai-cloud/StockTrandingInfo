...
async def generate_analysis(session, news_data):
    ...
    while True:
        ...
        [분석 가이드]
        1. 당신의 고객은 주식 단타를 통해 수익을 창출하는 사람입니다. 시장 전반의 요약과 향후 증시 예측하는데, 단타를 위한 고객을 위한 예측 위주로 해주세요. (에널리스트 톤).
        2. 각 종목의 거래량 급증 원인을 제공된 다수의 뉴스(최대 10개)를 종합하여 다각도로 분석해 주세요 (경제, 사회, 문화, 기술, 국제).
        ...
        try:
            ...
            log_error(f"AI 분석 중 오류 (Key #{state['current_index']})", e)
            if rotate_api_key(state): continue
            return None
